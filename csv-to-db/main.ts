...
function saveToDB(results: Array<CsvModel>): void {
    if(!results.length) {
    	return;
	}
    ...
    connection.connect((err) => {
        ...
        results.forEach((item, index) => {
            const sql = `INSERT INTO users (id, name, age, email, date, time, phone) ` +
                `VALUES (${+item.id}, \'${item.name}\', \'${item.age}\', \'${item.email}\', \'${item.date}\', \'${item.time}\', \'${item.phone}\')`;
